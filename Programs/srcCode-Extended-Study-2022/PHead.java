...
//    int row = 900;
    ...
    void writeExcel() {
        String fileName = "Y:\\2021job\\@tree\\测试PHead.xlsx";
        ((ExcelWriterSheetBuilder)EasyExcel.write(fileName).sheet("PHead模板").registerWriteHandler(new PHeadRecordBorrowExcelHandler())).doWrite(this.dataList());
    }
...
//        int ma = 1;
        ...
        for(int i = 1; i < this.row; ++i) {
            ...
            for(int j = ma+1; j < this.max; ++j) {
                a = Integer.valueOf(j);
                String stmp=a+"";
                if (0==flag && this.numIsP(Integer.valueOf(j))) {
                    flag=1;
                    stmp = "P" + stmp;
                }else if (1==flag && this.numIsP(Integer.valueOf(j))) {
                    flag=0;
                    break;
                }
            ...
            ma = a-1;//            https://blog.csdn.net/qq_38974638/article/details/117395831
...
class PHeadRecordBorrowExcelHandler implements CellWriteHandler {

    PHeadRecordBorrowExcelHandler() {
    }
