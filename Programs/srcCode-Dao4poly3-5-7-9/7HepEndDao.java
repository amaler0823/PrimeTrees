...
//    int row = 900;
    ...
    public static void main(String[] args) {
        HepEnd hepEnd = new HepEnd();
        hepEnd.writeExcel();
    }

    void writeExcel() {
        String fileName = "Y:\\2021job\\@tree\\测试HeppppEnd.xlsx";
        ((ExcelWriterSheetBuilder)EasyExcel.write(fileName).sheet("HeppppEnd模板").registerWriteHandler(new HepppEndRecordBorrowExcelHandler())).doWrite(this.dataList());
    }
    ...
    Boolean numIsInZ(int num) {//0- 35,57,84

        //0，6，8，18，21，35，39，57，62,
        int z = 0;
        int a = 6;
        int b = 2;
        int flag = 0;
    ...
    Boolean reltIsInt(int z, int num) {
        ...
        //-4ac
        BigInteger fac = BigInteger.valueOf(4).multiply(c).multiply(BigInteger.valueOf(5));
        //bb-4ac
        BigInteger bbfac = BigInteger.valueOf(((long) Math.pow(11, 2))).add(fac);
        ...
        if (BigInteger.ZERO.compareTo(bbfac) <= 0) {
            ...
            //-b +  sqrt bb-4ac
            BigDecimal dividend = dividendONE.add(BigDecimal.valueOf(11));
            //2a
...
class HepppEndRecordBorrowExcelHandler implements CellWriteHandler {

    HepppEndRecordBorrowExcelHandler() {
    }
